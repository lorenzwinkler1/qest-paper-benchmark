...
**Due to high memory consumption, this can fail in the docker container when a high number of parallel jobs is selected.** If this happens, maybe increase the memory of the container, or just run one job at once (`./run_empirical_bounds.sh 1`). A file failed, when you run `tsp` and the exit-code (E-level) is non-zero.
